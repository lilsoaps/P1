...
from .database import verify_user, add_user, get_user_password, get_username, check_password,clear_login_attempts,increment_login_attempts,check_login_attempts,update_password
    ...
    if request.method == 'POST':
        ...
        if not errors:
            if not verify_user(email):
                ...
                return render_template('sign_up.html', email_in_use=True, email=email, username=username , errors=errors)
        
        return render_template('sign_up.html', errors=errors, email=email, username=username, password=password, password_confirm=password_confirm)
    else:
...
def profile():
    ...
    return render_template('profile.html', email=session['user']['email'], username=session['user']['username'])
